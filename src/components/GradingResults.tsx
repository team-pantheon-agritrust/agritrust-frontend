...
export default function GradingResults() {
  ...
  if (!scanResult) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        <div className="text-lg font-semibold text-slate-900">No scan result found</div>
        <p className="text-sm text-slate-400">Please complete a grain scan first.</p>
        <button
  ...
  const gradeColors: Record<string, string> = { A: '#10B981', B: '#0EA5E9', C: '#D97706', D: '#EF4444' }
  ...
  return (
    <div>
      <div className="h-16 bg-white border-b border-slate-900/[0.06] flex items-center px-8 gap-4 sticky top-0 z-50">
        ...
        <div className="text-[17px] font-semibold tracking-[-0.01em] text-slate-900">AI Assessment Results</div>
      ...
      <div className="max-w-[720px] mx-auto p-10 px-8">
        ...
        <div className="rounded-[28px] p-8 text-white mb-6 relative overflow-hidden animate-fade-up"
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-start gap-[14px]">
              ...
              <div>
                <div className="text-2xl font-extrabold tracking-[-0.03em] mb-1">AI Assessment</div>
                <div className="text-sm text-white/50">Grade {ai.grade} · {ai.breakdown.defect_assessment} Defects · {ai.breakdown.moisture_assessment} Moisture</div>
                <div className="flex gap-2 mt-[10px]">
                  <span className="px-[10px] py-[3px] bg-white/10 rounded-full text-[11px] font-semibold text-white/60">{priceInfo.quantity}</span>
                  <span className="px-[10px] py-[3px] bg-white/10 rounded-full text-[11px] font-semibold text-white/60">Score: {ai.aiScore}</span>
                </div>
              </div>
            </div>
            <div className="text-right">
              <div className="text-xs text-white/40 font-medium">Total Amount</div>
              <div className="text-[32px] font-extrabold tracking-[-0.04em] text-white leading-none">{priceInfo.totalAmount}</div>
              <div className="text-xs text-white/40 mt-0.5">{priceInfo.unitPrice} / kg · {priceInfo.source}</div>
            </div>
          </div>
          ...
          <div className="p-3 px-4 bg-white/[0.06] rounded-[10px] border border-white/[0.08] mb-4">
            <div className="flex justify-between mb-2 items-center">
              <span className="text-xs text-white/45 font-medium">AI Confidence</span>
              <span className="text-sm font-bold text-[#34D399]">{ai.confidence}%</span>
            </div>
            <div className="h-[6px] bg-white/10 rounded-full overflow-hidden">
              <div className="h-full rounded-full" style={{ width: `${ai.confidence}%`, background: 'linear-gradient(90deg,#10B981,#34D399)' }} />
            </div>
          </div>
          ...
          {ai.reasoning && (
            <div className="p-3 px-4 bg-white/[0.04] rounded-[10px] border border-white/[0.06]">
              <div className="text-[11px] text-white/35 font-medium mb-1 uppercase tracking-[0.05em]">AI Reasoning</div>
              <p className="text-[13px] text-white/60 leading-[1.55]">{ai.reasoning}</p>
            </div>
          )}
        </div>

        {/* Breakdown */}
        <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden p-6 mb-6 animate-fade-up" style={{ animationDelay: '0.08s' }}>
          <div className="text-base font-bold tracking-[-0.02em] text-slate-900 mb-4">AI Assessment Breakdown</div>
          <div className="grid grid-cols-2 gap-3 mb-4">
            {[
              { label: 'Model Score',       value: String(ai.breakdown.model_score) },
              { label: 'Anomaly Score',     value: String(ai.breakdown.anomaly_score) },
              { label: 'Defect Assessment', value: ai.breakdown.defect_assessment },
              { label: 'Moisture',          value: ai.breakdown.moisture_assessment },
              { label: 'Dark Blobs',        value: String(ai.breakdown.dark_blobs_detected) },
              { label: 'AI Score',          value: String(ai.aiScore) },
            ].map(b => (
              <div key={b.label} className="p-3 bg-slate-50 rounded-[10px]">
                <div className="text-[11px] text-slate-400 font-medium mb-1">{b.label}</div>
          ...
          {ai.defects && ai.defects.length > 0 && (
            <div>
              <div className="text-[13px] font-semibold text-slate-900 mb-2">Detected Defects</div>
              <div className="flex flex-wrap gap-2">
                {ai.defects.map(d => (
                  <span key={d} className="px-[10px] py-[3px] bg-red-50 border border-red-100 rounded-full text-[12px] font-semibold text-red-600">{d}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {(!ai.defects || ai.defects.length === 0) && (
            <div className="p-3 px-[14px] bg-emerald-50 rounded-[10px] border border-[#A7F3D0] flex gap-[10px] items-start">
              ...
              <div className="text-[13px] text-emerald-900 leading-[1.5]">
                <strong>No defects detected.</strong> Grain sample passed all quality checks.
              </div>
            </div>
          )}
        </div>
        ...
        {/* Payment details */}
        <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden p-6 mb-6 animate-fade-up" style={{ animationDelay: '0.12s' }}>
          <div className="text-base font-bold tracking-[-0.02em] text-slate-900 mb-4">Payment Information</div>

          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="p-4 bg-slate-50 rounded-[10px]">
              <div className="text-[11px] text-slate-400 font-medium mb-1">Bank</div>
              <div className="text-[14px] font-bold text-slate-900">{paymentDetails.bank}</div>
            </div>
            <div className="p-4 bg-slate-50 rounded-[10px]">
              <div className="text-[11px] text-slate-400 font-medium mb-1">Account Number</div>
              <div className="text-[14px] font-bold text-slate-900 font-mono">{paymentDetails.accountNumber}</div>
            </div>
          ...
          <div className="p-3 px-4 bg-sky-50 border border-sky-100 rounded-[10px] text-[13px] text-sky-800">
            Transaction Ref: <span className="font-mono font-bold text-sky-900">{transactionRef}</span>
          </div>
        </div>

        {/* Actions */}
        <div className="grid grid-cols-2 gap-3 mb-4 animate-fade-up" style={{ animationDelay: '0.16s' }}>
          <button
