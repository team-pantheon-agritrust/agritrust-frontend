...
const scans = [
  { id: 1, emoji: '🌽', name: 'Yellow Maize',    date: '2 hours ago', location: 'Kano, NG',    grade: 'A', gradeColor: '#10B981', price: '₦87,500',  volume: '500 kg' },
  { id: 2, emoji: '🌾', name: 'Wheat — Hard Red', date: 'Yesterday',   location: 'Kaduna, NG', grade: 'B', gradeColor: '#0EA5E9', price: '₦64,200',  volume: '320 kg' },
  { id: 3, emoji: '🫘', name: 'Soybean',          date: '3 days ago',  location: 'Plateau, NG',grade: 'A', gradeColor: '#10B981', price: '₦112,000', volume: '800 kg' },
  { id: 4, emoji: '🌾', name: 'Sorghum',          date: '5 days ago',  location: 'Borno, NG',  grade: 'C', gradeColor: '#D97706', price: '₦38,400',  volume: '240 kg' },
]
    ...
    <div>
      ...
      <div className="p-8 max-w-[1200px]">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          ...
          <StatCard label="Total Scans"          value="48"   delta="4 pending"      iconBg="bg-amber-50"   iconColor="text-amber-600"   icon={<ScanCountIcon />} />
          <StatCard label="Avg Grade"            value="A–"   delta="Top 12%"     up iconBg="bg-violet-50"  iconColor="text-violet-600"  icon={<GradeIcon />}  />
        </div>
        ...
        <div className="grid gap-6 items-start" style={{ gridTemplateColumns: '1fr 340px' }}>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden p-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <div className="text-base font-bold tracking-[-0.02em] text-slate-900">Monthly Earnings</div>
                  <div className="text-xs text-slate-400 mt-0.5">Last 12 months</div>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-extrabold text-slate-900 tracking-[-0.03em]">₦3.6M</div>
                ...
                {scans.map(s => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-slate-900 tracking-[-0.01em]">{s.name}</div>
                      <div className="text-xs text-slate-400 mt-0.5">{s.date} · {s.location} · {s.volume}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        <div className="text-[13px] font-bold" style={{ color: s.gradeColor }}>Grade {s.grade}</div>
                        <div className="text-[11px] text-slate-400">Offered</div>
                      </div>
                      <div className="text-right min-w-[72px]">
                        <div className="text-[13px] font-semibold text-slate-900">{s.price}</div>
                      </div>
                      <ChevronIcon />
                    </div>
                  </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden p-5">
              ...
              {[
                { buyer: 'Lagos Grain Co.',  amount: '₦87,500',  items: 'Maize · 500kg',    status: 'Accepted', statusColor: '#10B981' },
                { buyer: 'Northgate Foods',  amount: '₦112,000', items: 'Soybean · 800kg',  status: 'Pending',  statusColor: '#D97706' },
              ].map(o => (
                <div key={o.buyer} className="py-3 border-b border-slate-50 last:border-b-0">
                  <div className="flex justify-between mb-1">
                    <div className="text-[13px] font-semibold text-slate-900">{o.buyer}</div>
                    <div className="text-sm font-bold text-slate-900">{o.amount}</div>
                  </div>
                  <div className="flex justify-between">
                    <div className="text-xs text-slate-400">{o.items}</div>
                    <div className="text-[11px] font-semibold" style={{ color: o.statusColor }}>{o.status}</div>
                  </div>
                </div>
              ))}
              <button
  ...
  return (
    <div className="p-6 bg-white border border-slate-900/[0.06] rounded-[20px]">
      <div className="flex items-start justify-between mb-3">
        <div className="text-xs font-medium tracking-[0.04em] uppercase text-slate-400">{label}</div>
        <div className={`w-[34px] h-[34px] rounded-lg flex items-center justify-center ${iconBg} ${iconColor}`}>
          {icon}
        </div>
      </div>
      <div className="text-[28px] font-bold tracking-[-0.03em] text-slate-900 leading-none mb-[6px]">{value}</div>
      <div className={`text-xs font-medium flex items-center gap-1 ${up ? 'text-emerald-500' : 'text-slate-400'}`}>
        {up && <span>↑</span>}
        {delta}
      </div>
    </div>
...
function ListingIcon() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 4h10M3 8h7M3 12h5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
}
function MoneyIcon() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.5" /><path d="M8 5v6M6 7h3a1 1 0 010 2H6" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" /></svg>
}
function ScanCountIcon() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 5V3a1 1 0 011-1h2M11 2h2a1 1 0 011 1v2M14 11v2a1 1 0 01-1 1h-2M5 14H3a1 1 0 01-1-1v-2M4 8h8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg>
}
function GradeIcon() {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 1l1.8 3.6L14 5.3l-3 2.9.7 4.1L8 10.4l-3.7 1.9.7-4.1-3-2.9 4.2-.7L8 1z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round" /></svg>
}
function ChevronIcon() {
  return <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-slate-300 shrink-0"><path d="M5 10l4-3-4-3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
}
function CameraIcon() {
  return <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M1 4.5A1.5 1.5 0 012.5 3h.77l.74-1h5l.74 1h.77A1.5 1.5 0 0113 4.5v6A1.5 1.5 0 0111.5 12h-9A1.5 1.5 0 011 10.5v-6z" stroke="currentColor" strokeWidth="1.3" /><circle cx="7" cy="7.5" r="2" stroke="currentColor" strokeWidth="1.3" /></svg>
}
