...
export default function FarmerDashboard() {
  ...
  const { farmerProfile, scanResult, transactionRef, transactionStatus } = useAppContext();
    ...
    <div>
      ...
      <div className="p-4 sm:p-8 max-w-[1200px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <StatCard
            label="Total Trades"
            value={trustLoading ? "—" : trustData ? `${trustData.totalTrades}` : "0"}
            delta={trustData ? `${trustData.honestTrades} honest` : "No trades yet"}
          ...
          <StatCard
            label="Honest Trades"
            value={trustLoading ? "—" : trustData ? `${trustData.honestTrades}` : "0"}
            delta={
          ...
          <StatCard
            label="Platform Fee"
            value={trustLoading ? "—" : trustData ? `${trustData.platformFeePercent}%` : "3%"}
            delta={
              trustData
        ...
        <div className="grid gap-6 items-start lg:grid-cols-[minmax(0,1fr)_340px]">
              ...
              <div className="px-5 pt-5">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
                  <div>
                    ...
                    <div className="text-xs text-slate-400 mt-0.5">This session</div>
                  </div>
                  <button
              ...
              <div className="px-2 pb-2">
                {scanResult ? (
                    ...
                    <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start">
                      <div className="text-right">
                        <div
                          ...
                          style={{ color: GRADE_COLOR[scanResult.ai.grade] ?? "#64748b" }}
                        >
                          Grade {scanResult.ai.grade}
                        </div>
                        <div className="text-[11px] text-slate-400">{scanResult.ai.confidence}% confidence</div>
                      </div>
                      <div className="text-right min-w-[80px]">
                        <div className="text-[13px] font-semibold text-slate-900">{scanResult.priceInfo.totalAmount}</div>
                        <div className="text-[11px] text-slate-400">{scanResult.priceInfo.unitPrice}/kg</div>
                      </div>
                      <ChevronIcon />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-3 py-10 px-4 text-center">
                    <div className="text-sm font-semibold text-slate-900">No scans yet</div>
                    <div className="text-xs text-slate-400">Scan your first grain batch to see results here</div>
                    <button
                      className="mt-1 inline-flex items-center gap-2 py-[8px] px-5 text-[13px] font-semibold rounded-[10px] border-0 bg-sky-500 text-white cursor-pointer hover:bg-sky-600 transition-colors"
                      onClick={() => navigate("/farmer/scan")}
                    >
                      Start scanning
                    </button>
            ...
            {transactionRef && scanResult && (
              <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden p-5">
                <div className="flex items-center justify-between mb-4">
                  <div className="text-base font-bold tracking-[-0.02em] text-slate-900">Current Escrow</div>
                  {statusLabel && (
                    <span className={`inline-flex items-center gap-[5px] px-[10px] py-[3px] text-[11px] font-semibold rounded-full ${statusLabel.cls}`}>
                      {statusLabel.text}
                    </span>
                  )}
                </div>
                <div className="flex flex-col gap-3">
                  <div className="flex justify-between items-center py-2 border-b border-slate-50">
                    ...
                    <span className="text-xs font-mono font-semibold text-slate-700">{transactionRef}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-slate-50">
                    <span className="text-xs text-slate-400">Amount</span>
                    <span className="text-sm font-bold text-slate-900">{scanResult.priceInfo.totalAmount}</span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-slate-50">
                    <span className="text-xs text-slate-400">Bank</span>
                    <span className="text-xs font-semibold text-slate-700">{scanResult.paymentDetails.bank}</span>
                  </div>
                  <div className="flex justify-between items-center py-2">
                    <span className="text-xs text-slate-400">Account</span>
                    <span className="text-xs font-mono font-semibold text-slate-700">{scanResult.paymentDetails.accountNumber}</span>
                  </div>
                </div>
                <div className="mt-4 px-3 py-2 bg-sky-50 rounded-[8px] text-xs text-sky-700">
          ...
          <div className="flex flex-col gap-5">
            ...
            >
              ...
              <svg width="160" height="160" viewBox="0 0 120 120" className="overflow-visible">
                ...
                <text x="60" y="56" textAnchor="middle"
                  style={{ fontSize: 38, fontWeight: 800, letterSpacing: "-0.04em", fill: "#0F172A", fontFamily: "Inter, sans-serif" }}>
                  {trustScore || "—"}
                </text>
                <text x="60" y="70" textAnchor="middle"
                  style={{ fontSize: 12, fill: "#94A3B8", fontFamily: "Inter, sans-serif" }}>
              ...
              {trustData ? (
                ...
                <div className="text-xs text-slate-400 mt-3">
                  {trustLoading ? "Fetching trust data…" : "Complete your profile to see score"}
                </div>
              )}
            ...
            {/* Quick actions */}
            <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden p-5">
              <div className="text-base font-bold tracking-[-0.02em] text-slate-900 mb-[14px]">Quick Actions</div>
              <div className="flex flex-col gap-2">
                <button
                  ...
                  onClick={() => navigate("/farmer/scan")}
                >
                  <span className="w-7 h-7 rounded-[8px] bg-sky-50 text-sky-500 flex items-center justify-center shrink-0"><CameraIcon /></span>
                  Scan grain
                </button>
                {scanResult && (
                  <button
                    ...
                    onClick={() => navigate("/farmer/results")}
                  >
                    <span className="w-7 h-7 rounded-[8px] bg-emerald-50 text-emerald-500 flex items-center justify-center shrink-0">
                ...
                <button
                  ...
                  onClick={() => navigate("/farmer/profile")}
                >
                  <span className="w-7 h-7 rounded-[8px] bg-violet-50 text-violet-500 flex items-center justify-center shrink-0"><ShieldIcon /></span>
                  Trust profile
                </button>
              </div>
            </div>
  ...
  return (
    <div className="p-6 bg-white border border-slate-900/[0.06] rounded-[20px]">
      <div className="flex items-start justify-between mb-3">
        ...
        <div className={`w-[34px] h-[34px] rounded-lg flex items-center justify-center ${iconBg} ${iconColor}`}>{icon}</div>
      </div>
...
function ShieldIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <path d="M8 1.5l5 2v4c0 3-2.2 5.2-5 6-2.8-.8-5-3-5-6v-4l5-2z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/>
    </svg>
  );
}
function MoneyIcon() {
...
function ScanCountIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      <path d="M5 3.5L8 2l3 1.5v4L8 9 5 7.5v-4zM8 9v5M5.5 12h5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
}
function GradeIcon() {
...
function ChevronIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-slate-300 shrink-0">
