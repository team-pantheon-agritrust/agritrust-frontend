...
const offers = [
  ...
  { id: 2, emoji: '🫘', name: 'Soybean',          farmer: 'Emmanuel Obi · Plateau',   grade: 'A', price: '₦112,000', volume: '800 kg',   status: 'Pending',    statusCls: 'bg-amber-100 text-amber-600',   age: '5h ago' },
  { id: 3, emoji: '🌾', name: 'Hard Red Wheat',   farmer: 'Hauwa Bello · Kaduna',     grade: 'B', price: '₦64,200',  volume: '320 kg',   status: 'Verified',   statusCls: 'bg-emerald-50 text-emerald-700', age: '1d ago' },
  { id: 4, emoji: '🌾', name: 'White Sorghum',    farmer: 'Daniel Lawan · Borno',     grade: 'B', price: '₦52,800',  volume: '400 kg',   status: 'In Transit', statusCls: 'bg-sky-100 text-sky-600',        age: '2d ago' },
  { id: 5, emoji: '🍚', name: 'Paddy Rice',       farmer: 'Fatima Usman · Kebbi',     grade: 'A', price: '₦138,000', volume: '1,000 kg', status: 'Pending',    statusCls: 'bg-amber-100 text-amber-600',   age: '3d ago' },
]
      ...
      <div className="p-8 max-w-[1200px]">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: 'In Escrow',         value: '₦199K', sub: '2 transactions',    dot: '#0EA5E9' },
            { label: 'Delivered (YTD)',   value: '18',    sub: '14.2 tonnes total', dot: '#10B981' },
            { label: 'Avg Quality Score', value: '91.4',  sub: 'A-grade average',   dot: '#7C3AED' },
          ].map(s => (
            <div key={s.label} className="p-6 bg-white border border-slate-900/[0.06] rounded-[20px]">
              <div className="flex items-start justify-between mb-3">
                <div className="text-xs font-medium tracking-[0.04em] uppercase text-slate-400">{s.label}</div>
                <div className="w-2 h-2 rounded-full mt-[3px]" style={{ background: s.dot }} />
              </div>
              <div className="text-[28px] font-bold tracking-[-0.03em] text-slate-900 leading-none mb-[6px]">{s.value}</div>
            ...
            <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden">
              <div className="px-5 pt-5">
                <div className="flex items-center justify-between mb-5">
                  <div className="text-base font-bold tracking-[-0.02em] text-slate-900">Incoming Offers</div>
                    ...
                    {['All', 'Verified', 'Pending', 'In Transit'].map(f => (
                      <button
                        ...
                        className={`px-3 py-1 rounded-full border border-slate-200 text-xs font-semibold cursor-pointer transition-all ${
                          f === 'All' ? 'text-slate-900 bg-slate-100' : 'text-slate-400 bg-transparent hover:bg-slate-50'
                        }`}
                      >
                        {f}
                      </button>
                ...
                {offers.map(o => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-slate-900 tracking-[-0.01em]">{o.name}</div>
                      <div className="text-xs text-slate-400 mt-0.5">{o.farmer} · {o.age}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      <div>
                        <div className="flex gap-[6px] mb-1 justify-end">
                          <span className="inline-flex items-center gap-[5px] px-[10px] py-[3px] text-[11px] font-semibold rounded-full bg-slate-100 text-slate-600">Grade {o.grade}</span>
                          <span className={`inline-flex items-center gap-[5px] px-[10px] py-[3px] text-[11px] font-semibold rounded-full ${o.statusCls}`}>{o.status}</span>
                        </div>
                        <div className="flex gap-2 justify-end items-center">
                          <div className="text-xs text-slate-400">{o.volume}</div>
                          <div className="text-[15px] font-bold text-slate-900">{o.price}</div>
                        </div>
                      </div>
                      <div className="flex gap-[6px]">
                        <button
            ...
            <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden">
              <div className="h-40 relative overflow-hidden" style={{ background: 'linear-gradient(135deg,#e8f4f8 0%,#d4e8f0 100%)' }}>
                <div className="absolute inset-0"
                  style={{ backgroundImage: 'linear-gradient(rgba(14,165,233,0.08) 1px,transparent 1px),linear-gradient(90deg,rgba(14,165,233,0.08) 1px,transparent 1px)', backgroundSize: '24px 24px' }}
                />
                ...
                <div className="absolute top-3 right-3">
                  <span className="inline-flex items-center gap-[5px] px-[10px] py-[3px] text-[11px] font-semibold rounded-full bg-emerald-50 text-emerald-700">GPS Verified</span>
                </div>
              </div>
              <div className="p-4">
                <div className="text-sm font-bold text-slate-900 mb-2">Active Delivery · ESC-4821</div>
                <div className="flex items-center gap-2 text-[13px] text-slate-500">
                  <div className="w-2 h-2 rounded-full bg-sky-500 shrink-0" />
                  <span className="text-xs">Kano Farm Gate</span>
                  <div className="flex-1 h-px" style={{ background: 'linear-gradient(90deg,#0EA5E9,#10B981)' }} />
                  <span className="text-xs text-emerald-500 font-semibold">Lagos Warehouse</span>
                  <div className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />
                </div>
                <div className="mt-[10px] flex justify-between items-center">
                  <div>
                    <div className="text-xs text-slate-400">ETA</div>
                    <div className="text-sm font-bold text-slate-900">Tomorrow, 11:30 AM</div>
                  </div>
                  <div className="w-[120px] h-[6px] bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: '68%', background: 'linear-gradient(90deg,#0EA5E9,#10B981)' }} />
                  </div>
          ...
          <div className="flex flex-col gap-5">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-[6px] bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: `${e.pct}%`, background: 'linear-gradient(90deg,#0EA5E9,#10B981)' }} />
                    </div>
                    <div className="text-xs font-semibold text-sky-500">{e.pct}%</div>
                  </div>
                  <div className="text-[11px] text-slate-400 mt-1">Waiting: {e.condition}</div>
                </div>
              ))}
              <button
            ...
            <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden p-5">
              ...
              <div className="flex flex-col gap-2">
                {['Maize', 'Soybean', 'Wheat', 'Rice'].map(g => (
                  ...
                  >
                    <span>{g}</span>
                    <span className="text-[11px] text-slate-400">{Math.floor(Math.random() * 20 + 5)} listings →</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-900/[0.06] rounded-[20px] shadow-sm overflow-hidden p-5">
              ...
              {[
                ...
                <div key={g.grade} className="flex items-center gap-[10px] mb-[10px] last:mb-0">
                  <div className="w-6 h-6 rounded-[6px] flex items-center justify-center text-[11px] font-extrabold shrink-0"
                    style={{ background: g.color + '15', border: `1px solid ${g.color}30`, color: g.color }}>
                    {g.grade}
                  </div>
                  <div className="flex-1 h-[6px] bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${g.pct}%`, background: g.color }} />
                  </div>
                  <div className="text-xs font-bold text-slate-900 min-w-[28px]">{g.pct}%</div>
                </div>
              ))}
            </div>
          </div>
